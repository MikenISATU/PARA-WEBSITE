...
function FeatureCard({
  ...
  return (
    <div
      className={[
        'relative rounded-3xl p-7 flex flex-col gap-4 overflow-hidden transition-all duration-300 h-full',
        highlight
    ...
    >
      {highlight && (
        <>
          ...
          <span className="absolute top-5 right-5 text-[10px] font-bold uppercase tracking-widest bg-white/20 text-white rounded-full px-3 py-1 backdrop-blur-sm">
            Featured
          </span>
        </>
      )}
      ...
      <div className="relative rounded-[3.4rem] bg-gradient-to-br from-[#f8fafc] via-[#cbd5e1] to-[#94a3b8] p-[5px] shadow-[0_30px_90px_rgba(15,23,42,0.28)]">
        <div className="rounded-[3rem] bg-gradient-to-br from-[#dfe6ee] via-[#b8c2cf] to-[#8a96a8] p-[2px]">
          <div className="relative rounded-[2.8rem] bg-black p-[10px] overflow-hidden">
            ...
            <div className="relative overflow-hidden rounded-[2.35rem] bg-zinc-950 aspect-[1080/2464]">
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="space-y-3">
          {faqs.map((faq, i) => (
            <div
              key={i}
              className={['rounded-2xl border transition-all duration-200 overflow-hidden', open === i ? 'border-[#2563eb] shadow-sm' : 'border-gray-100'].join(' ')}
            >
              <button
...
function TeamModal({
  ...
  const displaySections = sections.map((s) => ({
    ...s,
    label: member.name === 'Keannu Torre' && s.label === 'Why I Joined PARA' ? 'Why I Started PARA' : s.label,
  }));
          ...
          {member.avatar ? (
            <img
              ...
              onError={(e) => {
                ...
                parent.innerHTML = `<div class="w-full h-full bg-gradient-to-br ${member.gradient} flex items-center justify-center"><span style="color:white;font-weight:700;font-size:3rem">${member.name
                  .split(' ')
                  .map((w: string) => w[0])
                  .join('')
                  .slice(0, 2)}</span></div>`;
              ...
              {member.avatar ? (
                <img
                  ...
                  onError={(e) => {
                    ...
                    t.style.display = 'none';
                    const parent = t.parentElement!;
                    parent.innerHTML = `<div class="w-full h-full bg-gradient-to-br ${member.gradient} flex items-center justify-center"><span style="color:white;font-weight:700;font-size:2.5rem">${member.name
                      .split(' ')
                      .map((w: string) => w[0])
                      .join('')
                      .slice(0, 2)}</span></div>`;
...
function NavbarWrapper() {
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      else setVisible(window.scrollY > 60);
    };
            ...
            <div className="text-center lg:text-left max-w-xl lg:max-w-none mx-auto lg:mx-0">
              <div className="flex items-center justify-center lg:justify-start -mb-8 lg:-mb-10">
                <Image
                  ...
                  height={200}
                  priority
                  className="w-auto max-w-full h-48 sm:h-56 lg:h-64 object-contain object-left lg:-ml-12 xl:-ml-14"
                />
              </div>
              ...
              <div className="mt-7 flex flex-col items-center lg:items-start gap-4">
                ...
                >
                  ...
                  <svg
                    ...
                    className="w-4 h-4 shrink-0 opacity-60 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform"
                  >
      ...
      <section id="how-it-works" className="py-20 px-6 bg-white scroll-mt-16">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-0 md:gap-8 relative">
            ...
            {[
              { step: '01', title: 'Enter your destination', desc: 'Where are you headed? Enter a street, landmark, or barangay.' },
              ...
              <div key={s.step} className="relative z-10 flex flex-col items-center text-center">
                ...
                <div className="w-20 h-20 rounded-full bg-[#2563eb] text-white text-2xl font-bold flex items-center justify-center shadow-lg mb-5 mt-5 md:mt-0">
                  {s.step}
                </div>
              ...
              <div className="space-y-8">
                {roadmapItems.map((item) => (
                  <div key={item.city} className="relative flex gap-6 pl-14">
                    ...
                    <div
                      className={[
                        'rounded-2xl border flex-1 overflow-hidden',
                        item.status === 'current'
                    ...
                    >
                      <div className="p-5">
                        ...
                        <h3 className={['font-semibold mt-1 mb-1', item.status === 'current' ? 'text-white' : 'text-gray-900'].join(' ')}>
                          {item.title}
                        </h3>

                        <p className={['text-sm', item.status === 'current' ? 'text-blue-100' : 'text-gray-500'].join(' ')}>
                          {item.desc}
                        </p>
            ...
            <div>
              ...
              <ul className="space-y-3">
                {[
                  ...
                  <li key={l.label}>
                    <button onClick={() => scrollTo(l.id)} className="text-white/55 hover:text-white text-sm transition-colors text-left">
                      {l.label}
                    </button>
                  </li>
                ))}
